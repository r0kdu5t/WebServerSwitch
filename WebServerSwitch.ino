/*
 WebServerSwitch.ino based upon EthernetSwitching02
 Reference: http://www.instructables.com/id/Ethernet-Switching-with-Arduino/?ALLSTEPS
 All strings are in FLASH so it fit also Arduino UNO RAM
 */
#include <Ethernet.h>
#include <SPI.h>
#include <EEPROM.h>
...
int outputQuantity = 6; //should not exceed 10 and I only need six (6)
...
int outputAddress[6] = {2,3,5,6,7,8}; // Allocate six (6) spaces and name the output pin address.
String buttonText[6] = {"01. TEST01","02. TEST02","03. TEST03","04. TEST04","05. TEST05","06. TEST06"};
// Set the output to retain the last status after power recycle.
int retainOutputStatus[6] = {0,0,0,0,0,0};//1-retain the last status. 0-will be off after power cut.
////////////////////////////////////////////////////////////////////////
...
//---------------------------------------------------------------------------------------------
			if( c == '1') {
				...
				switch (c) {
					case '0':triggerPin(outputAddress[0], client, outp);
					break;
					// case '1':
					// triggerPin(outputAddress[1], client, outp);
					// break;
